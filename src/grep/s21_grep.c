...
Flags *initFlags(int *errorCode) {
  ...
  if (!flags)
    *errorCode = errorCode_allocFailed;
  return flags;
}
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < 512; i++)
    pattern[i] = calloc(4096, sizeof(char));
  if (argc > 2) {
...
void workgrep(int argc, int index, char *argv[], Flags *flags, Counts *counts,
  ...
  if (flags->flag_c)
    flag_c(argc, index, argv, flags, counts);
}

int flag_i(Flags *flags) {
  int reg = REG_EXTENDED | REG_NEWLINE;
  if (flags->flag_i)
    reg = REG_EXTENDED | REG_ICASE | REG_NEWLINE;
  return reg;
}
